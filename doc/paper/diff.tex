...
%DIF DEL gcb_sub_vpd_et_paper.tex   Mon Apr  2 23:14:15 2018
%DIF ADD vpd_et_paper.tex           Mon Apr  2 23:11:05 2018
\documentclass[draft,linenumbers]{agujournal}
%DIF 3c3-4
%DIF < \draftfalse \usepackage{makecell} \usepackage{multirow} %AM for tables
%DIF -------
% \draftfalse %DIF > 
\usepackage{makecell} \usepackage{multirow} %AM for tables %DIF > 
%DIF -------
\usepackage{booktabs} \drafttrue

%DIF 6c7-8
%DIF < \journalname{Global Change Biology: When does VPD drive or reduce ET?}
%DIF -------
\journalname{Agricultural and Forest Meteorology: When does VPD drive %DIF > 
  or reduce ET?} %DIF > 
%DIF -------
...
\title{When does vapor pressure deficit drive or reduce
  evapotranspiration?\DIFdelbegin \footnote{\DIFdel{Primary Research Article}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend }


\DIFdelbegin %DIFDELCMD < \authors{Adam Massmann\affil{1}, Pierre Gentine\affil{1}, Changjie
%DIFDELCMD < Lin\affil{1,2}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \authors{Adam Massmann\affil{a}, Pierre Gentine\affil{a}, Changjie
  Lin\affil{a,b}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \affiliation{1}{Department of Earth and Environmental Engineering,
%DIFDELCMD < Columbia University, New York, NY 10027} \affiliation{2}{State Key
%DIFDELCMD < Laboratory of Hydroscience and Engineering, Department of Hydraulic
%DIFDELCMD < Engineering, Tsinghua University, Beijing, CN 100084}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affiliation{a}{Department of Earth and Environmental Engineering,
...
\begin{abstract}
  \DIFdelbegin \DIFdel{Increases to }\DIFdelend \DIFaddbegin \DIFadd{Increasing }\DIFaddend vapor pressure deficit \DIFdelbegin \DIFdel{(VPD) and
}\DIFdelend \DIFaddbegin \DIFadd{increases }\DIFaddend atmospheric demand for
  water\DIFdelbegin \DIFdel{are expected with rising atmospheric
}\DIFdelend \DIFaddbegin \DIFadd{, and vapor pressure deficit is expected to rise with
  increasing }\DIFaddend CO$_2$. While increased evapotranspiration (ET) in
  ...
  \DIFaddend The theory suggests that for most environmental conditions and plant
  ...
  increasing). However for forest, grass, \DIFaddbegin \DIFadd{savannah, }\DIFaddend and shrub plant
  types, ET more frequently decreases than increases with rising
  VPD. \DIFdelbegin \DIFdel{Therefore, we may
expect reductions in ET due to CO$_2$-induced increases in VPD. }\DIFdelend This work serves as an example of the utility of
...
}\DIFaddend .  In other words, the question ``When does
...
the soil and atmosphere. If \DIFaddbegin \DIFadd{ecosystem }\DIFaddend plant response reduces ET with
...
standpoint. \DIFaddbegin \DIFadd{As a counterpoint, one may argue that increases in ET with
...
environmental conditions could cause a decrease in the likelihood
of precipitation \mbox{%DIFAUXCMD
\citep[][]{Gentine_2013}}%DIFAUXCMD
. Furthermore, any increases
in precipitation are likely to be non-local, such that plants giving
up water to the atmosphere are not guaranteed to reap the benefits of
water returned from the atmosphere to the soil. Using this logic, from
an ecosystem evolutionary perspective, water stored in soil is worth
much more than the chance of water returned as precipitation.
}\DIFaddend 

We can use intuition about plant water conservation strategy to
hypothesize about ET response to changes in VPD. Plants \DIFaddbegin \DIFadd{and ecosystems
}\DIFaddend that evolved to conserve water, such as arid shrubs \DIFaddbegin \DIFadd{or savannah}\DIFaddend ,
...
The objective of the present manuscript is to \DIFdelbegin \DIFdel{evaluate the VPD
dependence of ET, in non-extreme soil drought conditions. The goal of
this paper is to }\DIFdelend use reasonable
approximations as a tool to develop intuition for plant response to
atmospheric drying \DIFaddbegin \DIFadd{and evaluate the VPD dependence of ET}\DIFaddend . This
intuition will aid interpretation of observations\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend full complexity
models\DIFdelbegin \DIFdel{. In
order to quantify plant response to perturbations in VPD, we apply a Penman-Monteith framework }\DIFdelend \DIFaddbegin \DIFadd{, and facilitate the disentanglement of complex land-atmosphere
...
\DIFadd{Recent results have drastically improved our understanding of VPD's
impact on physiology. \mbox{%DIFAUXCMD
\citet{MEDLYN_2011} }%DIFAUXCMD
developed a model for
leaf-scale stomatal conductance ($g_s$), including VPD response, by
combining an optimal photosynthesis theory \mbox{%DIFAUXCMD
\citep{Farquhar_1980,
  Katul_2010} }%DIFAUXCMD
with an empirical approach, and extended use of this
model to the ecosystem scale in \mbox{%DIFAUXCMD
\citet{Medlyn_2017}}%DIFAUXCMD
. Additionally,
\mbox{%DIFAUXCMD
\citet{Zhou_2014} }%DIFAUXCMD
demonstrated that a quantity underlying water use
...
season across space and time, within a PFT \mbox{%DIFAUXCMD
\citep{Zhou_2015}}%DIFAUXCMD
. These
results provide novel tools to examine how ET varies in response to
VPD, while explicitly including VPD's effect on stomatal conductance
for the first time. In this manuscript, we leverage these tools }\DIFaddend to
derive the theoretical response of ET to VPD \DIFdelbegin \DIFdel{. The model is then }\DIFdelend \DIFaddbegin \DIFadd{using a Penman Monteith
framework. Our theory is }\DIFaddend validated and tested at multiple
eddy-covariance stations spanning various climates and plant
functional types.

\section{Materials and Methods}
\DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{methods}%%%
%DIFDELCMD < %%%
\DIFdelend \subsection{Data}
\label{data}
We use both meteorological and eddy-covariance data from the
FLUXNET2015 database \DIFdelbegin \DIFdel{, including all sites with at least four years
of data }\DIFdelend (data available at \sloppy
https://fluxnet.fluxdata.org/data/fluxnet2015-dataset/ \sloppy)\DIFdelbegin \DIFdel{. Each
site's plant functional type }\DIFdelend \DIFaddbegin \DIFadd{,
...
grouped into nine plant functional types }\DIFaddend (PFT) \DIFdelbegin \DIFdel{was classified using }\DIFdelend \DIFaddbegin \DIFadd{according to }\DIFaddend the
International Geosphere-Biosphere Programme vegetation classification
scheme \citep{Loveland_1999}\DIFdelbegin \DIFdel{. The physical constants used in the
stomatal conductance model in the Methods section (Section
\ref{methods}) are only published for six plant functional types
(PFTs): crops }\DIFdelend \DIFaddbegin \DIFadd{: cropland }\DIFaddend (CRO), \DIFaddbegin \DIFadd{grass (GRA), }\DIFaddend deciduous
broadleaf forest (DBF), evergreen broadleaf \DIFdelbegin \DIFdel{forests }\DIFdelend \DIFaddbegin \DIFadd{forest }\DIFaddend (EBF), evergreen
needleleaf forest (ENF), \DIFdelbegin \DIFdel{grass
(GRA), and }\DIFdelend \DIFaddbegin \DIFadd{mixed forest (MF), }\DIFaddend closed shrub (CSH)\DIFdelbegin \DIFdel{(see Table \ref{pft}). There are 62
sites with these plant functional types, and their location is shown
}\DIFdelend \DIFaddbegin \DIFadd{,
savannah (SAV), and woody savannah (WSA) (site locations }\DIFaddend in Figure
\ref{map_fig}\DIFaddbegin \DIFadd{)}\DIFaddend .
...
coefficient (from \mbox{%DIFAUXCMD
\citet{Lin_2015}}%DIFAUXCMD
, via \mbox{%DIFAUXCMD
\citet{Franks_2017}}%DIFAUXCMD
), and
calculated uWUE. uWUE from \mbox{%DIFAUXCMD
...
1-8 with the variables defined Table \ref{definitions}.}}  %DIFAUXCMD
%DIFDELCMD < \small
%DIFDELCMD < %DIFDELCMD < \label{pft} %%%
\centering
%DIFDELCMD < \begin{tabular}{l c c @{\qquad} c c} \hline
%DIFDELCMD < \multirow{2}[3]{*}{Abbreviation} & \multirow{2}[3]{*}{PFT} &
%DIFDELCMD < \multirow{2}[3]{*}{$g_1$ (Pa$^{0.5}$)} & \multicolumn{2}{c}{uWUE
%DIFDELCMD < ($\mu$-mol [C] Pa$^{0.5}$ J$^{-1}$ [ET])} \\ \cmidrule{4-5}
%DIFDELCMD < 
...
\item We remove days with rain and the day following to avoid issues
  ...
  humidity (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{MEDLYN_2011}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\citet{Medlyn_2017} }%DIFAUXCMD
    ...
    \DIFaddbeginFL \DIFaddFL{$g_{1}$  }& \DIFaddFL{ecosystem-scale slope parameter }& \DIFaddFL{Pa$^{0.5}$ }& \DIFaddFL{\mbox{%DIFAUXCMD
\citet{MEDLYN_2017} }%DIFAUXCMD
}\\
    \DIFaddendFL $R$ & universal gas constant & J mol$^{-1}$ K$^{-1}$ & - \\
    $R_{air}$ & gas constant of air & J  K$^{-1}$ kg$^{-1}$ & - \\
    \DIFdelbeginFL \DIFdelFL{$LAI$ }%DIFDELCMD < & %%%
\DIFdelFL{leaf area index }%DIFDELCMD < & %%%
\DIFdelFL{-}%DIFDELCMD < & %%%
\DIFdelFL{- }%DIFDELCMD < \\ %%%
\DIFdelendFL $\sigma$ & uncertainty parameter & -& - \\
...
by combining an optimal photosynthesis theory \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Farquhar_1980,
Katul_2010} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Cowan_1977} }%DIFAUXCMD
}\DIFaddend with an empirical approach, which describes the
dependence of $g_s$ to VPD. This resulted in the following model for
leaf-scale stomatal conductance:

\begin{linenomath*}
  \begin{equation}
    g\DIFdelbegin \DIFdel{_{l-s} }\DIFdelend \DIFaddbegin \DIFadd{_{s-leaf} }\DIFaddend = g_0 + 1.6 \left(1 +
      \DIFdelbegin \DIFdel{\frac{g_1}{\sqrt{VPD}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{g_{1-leaf}}{\sqrt{VPD}}}\DIFaddend \right) \frac{A}{c_a}\DIFaddbegin \DIFadd{,
    }\label{leaf_medlyn}
  \DIFaddend \end{equation}
\end{linenomath*}
\DIFdelbegin \DIFdel{Where $g_1$ }\DIFdelend \DIFaddbegin \DIFadd{where $g_{1-leaf}$ }\DIFaddend is a leaf-scale ``slope'' parameter, A is the net
...
parameter (\DIFdelbegin \DIFdel{$g_1$}\DIFdelend \DIFaddbegin \DIFadd{$g_{1-leaf}$}\DIFaddend ) to physical parameters as:
\begin{linenomath*}
  \label{slope}
  \begin{equation}
    g\DIFdelbegin \DIFdel{_1 }\DIFdelend \DIFaddbegin \DIFadd{_{1-leaf} }\DIFaddend = \sqrt{\frac{3 \, \Gamma^* \, \lambda}{1.6}},\footnote{Note this expression has units of of (mmol mol$^{-1}$)$^{1/2}$, but this can be converted to Pa$^{1/2}$ using the ideal gas law.}
  \end{equation}
\end{linenomath*}
...
\DIFdelbegin \DIFdel{$g_1$ }\DIFdelend \DIFaddbegin \DIFadd{$g_{1-leaf}$ }\DIFaddend is a leaf-scale term reflecting the \DIFdelbegin \DIFdel{tradeoff }\DIFdelend \DIFaddbegin \DIFadd{trade-off }\DIFaddend of water for
carbon uptake\DIFdelbegin \DIFdel{and growth}\DIFdelend . The higher \DIFdelbegin \DIFdel{$g_1$}\DIFdelend \DIFaddbegin \DIFadd{$g_{1-leaf}$}\DIFaddend , the more open the stomata and
the more they release water in exchange for carbon.


The Medlyn model for stomatal conductance has been shown to behave
very well across PFTs \citep[][]{Lin_2015}, and \DIFdelbegin \DIFdel{can be adapted to the
ecosystem scale by multiplying $g_{l-s}$ by leaf area index (LAI) and
converting units
to }\DIFdelend \DIFaddbegin \DIFadd{has been successfully
adopted to ecosystem scale analysis in \mbox{%DIFAUXCMD
\citet{Medlyn_2017}}%DIFAUXCMD
. In units
of }\DIFaddend m s$^{-1}$\DIFdelbegin \DIFdel{with the ideal gas law}\DIFdelend \DIFaddbegin \DIFadd{, the ecosystem scale stomatal conductance is}\DIFaddend :

\begin{linenomath*}
  \begin{equation}
    g_s = \DIFdelbegin \DIFdel{\text{LAI} }\DIFdelend \frac{R \,T}{P} \DIFdelbegin %DIFDELCMD < \left( %%%
\DIFdel{g_0 + }\DIFdelend \DIFaddbegin \DIFadd{\; }\DIFaddend 1.6 \left(1 + \frac{g_1}{\sqrt{VPD}}\right) \DIFdelbegin \DIFdel{\frac{A}{c_a}}%DIFDELCMD < \right)
%DIFDELCMD <       %%%
\DIFdelend \DIFaddbegin \DIFadd{\frac{GPP}{c_a},
    }\DIFaddend \label{medlyn}
  \end{equation}
\end{linenomath*}

\DIFaddbegin \DIFadd{where GPP is the ecosystem scale gross primary production, and $g_1$
is an ecosystem scale analogue to $g_{1-leaf}$. We solve for $g_1$
following \mbox{%DIFAUXCMD
\citet{Medlyn_2017} }%DIFAUXCMD
(Equation 5), and take the median $g_1$
...
\DIFaddend While Equation 3 can be used in PM (Equation 1), it will make
analytical work with the function intractable because \DIFdelbegin \DIFdel{$A$, net CO$_2$
assimilation rate, }\DIFdelend \DIFaddbegin \DIFadd{$GPP$ }\DIFaddend is
functionally related to ET itself. \DIFdelbegin \DIFdel{To remove the
dependence of ET on $A$ we use fundamental }\DIFdelend \DIFaddbegin \DIFadd{Additionally, a perturbation to VPD
...
conductance. We can use aforementioned }\DIFaddend semi-empirical results of
\citet{Zhou_2015} \DIFdelbegin \DIFdel{, who showed that  the }\DIFdelend \DIFaddbegin \DIFadd{as a tool to approach this
problem. \mbox{%DIFAUXCMD
\citet{Zhou_2015}}%DIFAUXCMD
, showed that  }\DIFaddend underlying Water Use
Efficiency (uWUE):
...
\DIFaddend is relatively constant across time and moisture conditions within a
plant functional type\DIFdelbegin \DIFdel{. If, following
\mbox{%DIFAUXCMD
\citet{Lin_2015}}%DIFAUXCMD
, we approximate $g_0$ as $0$ (i.e. we neglect
cuticular and epidermal losses - a reasonable assumption except in
very dry conditions), we }\DIFdelend \DIFaddbegin \DIFadd{, and correctly captures a constant relationship
between GPP, ET and VPD over a diurnal cycle \mbox{%DIFAUXCMD
\citep{Zhou_2014}}%DIFAUXCMD
. We }\DIFaddend can
use uWUE to remove the \DIFdelbegin \DIFdel{$A$ }\DIFdelend \DIFaddbegin \DIFadd{$GPP$ }\DIFaddend dependence from $g_s$ in a way that makes
...
\DIFdelbegin \DIFdel{Note that uWUE is fit on the ecosystem scale as in \mbox{%DIFAUXCMD
\citet{Zhou_2015}}%DIFAUXCMD
,
so GPP in Equation \ref{uwue} is really $A \cdot \text{LAI}$. This
leads to the cancellation of LAI in the step from Equation
\ref{medlyn} to Equation \ref{new_g_s}. }\DIFdelend Plugging Equation \ref{new_g_s} into Equation \ref{orig_pen} and
rearranging gives a new explicit expression for PM, in which
dependence on \DIFdelbegin \DIFdel{$A$ }\DIFdelend \DIFaddbegin \DIFadd{$GPP$ }\DIFaddend is removed:
...
With Equation \ref{et} \DIFdelbegin \DIFdel{, ET is explicitly a }\DIFdelend \DIFaddbegin \DIFadd{we have derived for the first time, using
recent results \mbox{%DIFAUXCMD
\citep[][]{MEDLYN_2011, Zhou_2014, Zhou_2015,
  Medlyn_2017}}%DIFAUXCMD
, ET explicitly as }\DIFaddend function of environmental variables
...
is \DIFdelbegin \DIFdel{a leaf-scale term }\DIFdelend related to the willingness of stomata to trade water for CO$_2$ and
to keep stomata open. uWUE is a semi-empirical ecosystem-scale
constant related to how WUE changes with VPD (specifically
$VPD^{-1/2}$). \DIFdelbegin \DIFdel{Is }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend is also roughly proportional to physical constants:
...
for \DIFdelbegin \DIFdel{the two parameters $g_1$, and uWUE. $g_1$ has been measured and
reported for the PFTs considered here \mbox{%DIFAUXCMD
\citep{Lin_2015, Franks_2017}}%DIFAUXCMD
, and we fit }\DIFdelend uWUE\DIFaddbegin \DIFadd{, which we fit }\DIFaddend by calculating its expectation, given the model
...
variations of uWUE \DIFaddbegin \DIFadd{and $g_1$}\DIFaddend ), we introduce an uncertainty parameter
...
through any uncertainty to our \DIFaddbegin \DIFadd{partial }\DIFaddend derivative of Equation
\ref{et_sigma} \DIFaddbegin \DIFadd{with respect to VPD}\DIFaddend :
...
response to atmospheric demand perturbations \DIFdelbegin \DIFdel{, with an implicit
assumption of non-extreme drought conditions }\DIFdelend \DIFaddbegin \DIFadd{with environmental
conditions held fixed}\DIFaddend . There are a few subtleties to taking the
...
\DIFaddbegin \DIFadd{This derivation relied either implicitly or explicitly on several
assumptions. First, the underlying theory used to derive Equations
\ref{leaf_medlyn} and \ref{medlyn} rely on an implicit assumption that
...
\mbox{%DIFAUXCMD
\citep{De_2017, Medlyn_2017}}%DIFAUXCMD
. Given our focus on the growing season,
which is usually characterized by relatively high insolation inducing
instability and convective boundary layers, we would
expect the surface to be generally well coupled. Additionally, by following
\mbox{%DIFAUXCMD
\citet{Medlyn_2017}}%DIFAUXCMD
's method for calculating ecosystem scale $g_1$
using PM,
...
theory used to develop stomatal conductance ($g_s$). An additional
assumption in the formulation of $uWUE$
\mbox{%DIFAUXCMD
\citep{Zhou_2014, Zhou_2015} }%DIFAUXCMD
and
\mbox{%DIFAUXCMD
\citet{Medlyn_2017}}%DIFAUXCMD
's stomatal conductance model is that
direct soil evaporation (E) contributions to ET remain
small relative to transpiration (T). This should be more true during the growing season. However, some plant types allow for systematically larger
contributions of ``E'' in ET, particularly those with less dense
canopies and smaller absolute amounts of transpiration. We therefore
...
tree PFTs \mbox{%DIFAUXCMD
\citep{Lin_2015}}%DIFAUXCMD
. Given these strong assumptions made with
...
\ref{testing}) and examining sensitivity to the exponent of the VPD
response, which is derived using assumptions that $ET=g_s \cdot VPD$ (Section
\ref{functional_form}).
}

\DIFaddend We note one final comment on our derivation which is relevant for
...
published \DIFdelbegin \DIFdel{in \mbox{%DIFAUXCMD
\citet{Zhou_2015} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Lin_2015}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{here}\DIFaddend . This makes Equation \ref{et} a useful alternative to
...
\DIFaddbegin \DIFadd{(assuming some relationship between $\lambda$ and }[\DIFadd{CO$_2$}]\DIFadd{)}\DIFaddend , contrary
to typical drought indices such as the Palmer Drought Severity Index
(PDSI) \citep{Swann_2016}. \DIFaddbegin \DIFadd{Lastly, all code and data used in this
analysis, including those used to generate the figures and tables, are
publicly available at https://github.com/massma/climate\_et.
}\DIFaddend 
...
dependence of $\sigma$ on VPD in their joint \DIFdelbegin \DIFdel{distrubution}\DIFdelend \DIFaddbegin \DIFadd{distribution}\DIFaddend , and
...
\DIFdelFL{376.0 }%DIFDELCMD < &
%DIFDELCMD < %%%
\DIFdelFL{65.2 }%DIFDELCMD < & %%%
\textbf{\DIFdelFL{936.6}} %DIFAUXCMD
%DIFDELCMD < \\ %%%
\DIFdelFL{CSH }%DIFDELCMD < & %%%
\DIFdelFL{289.0 }%DIFDELCMD < & %%%
\DIFdelFL{383.6 }%DIFDELCMD < & %%%
\DIFdelFL{67.5 }%DIFDELCMD < & %%%
\textbf{\DIFdelFL{12852.5}}
%DIFAUXCMD
%DIFDELCMD < \\ %%%
\DIFdelFL{DBF }%DIFDELCMD < & %%%
\DIFdelFL{288.6 }%DIFDELCMD < & %%%
\DIFdelFL{379.5 }%DIFDELCMD < & %%%
\DIFdelFL{63.6 }%DIFDELCMD < & %%%
\textbf{\DIFdelFL{1266.6}} %DIFAUXCMD
%DIFDELCMD < \\ %%%
\DIFdelFL{EBF }%DIFDELCMD < & %%%
\DIFdelFL{288.4 }%DIFDELCMD < & %%%
\DIFdelFL{368.5
}%DIFDELCMD < & %%%
\DIFdelFL{61.6 }%DIFDELCMD < & %%%
\textbf{\DIFdelFL{1454.8}} %DIFAUXCMD
%DIFDELCMD < \\ %%%
\DIFdelFL{ENF }%DIFDELCMD < & %%%
\DIFdelFL{288.28 }%DIFDELCMD < & %%%
\DIFdelFL{379.8 }%DIFDELCMD < & %%%
\DIFdelFL{61.0 }%DIFDELCMD < &
%DIFDELCMD < %%%
\textbf{\DIFdelFL{1853.7}} %DIFAUXCMD
%DIFDELCMD < \\ %%%
\DIFdelFL{GRA }%DIFDELCMD < & %%%
\DIFdelFL{288.4 }%DIFDELCMD < & %%%
\DIFdelFL{378.0 }%DIFDELCMD < & %%%
\DIFdelFL{61.0 }%DIFDELCMD < & %%%
\textbf{\DIFdelFL{3261.1}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \input{vpd_crit.tex}
    \DIFaddendFL \hline
  \end{tabular}
  \label{vpd_crit}
\end{table}
...
in $WUE$ causes more water loss per unit carbon \DIFdelbegin \DIFdel{production}\DIFdelend \DIFaddbegin \DIFadd{gain}\DIFaddend , and explains the
...
conservative closed shrub (CSH). CRO has \DIFdelbegin \DIFdel{high slope parameters and uWUE ($g_1 = 183.1$ }\DIFdelend \DIFaddbegin \DIFadd{higher slope parameter and a
slightly higher uWUE ($g_1 = 140.7$ }\DIFaddend Pa$^{1/2}$; \DIFdelbegin \DIFdel{$2.27$ }\DIFdelend \DIFaddbegin \DIFadd{$2.85$ }\DIFaddend $\mu$-mol [C]
Pa$^{0.5}$ J$^{-1}$ [ET]) compared to CSH (\DIFdelbegin \DIFdel{$g1 = 148.6 \, Pa^{1/2}$,
$uWUE=1.66$
}\DIFdelend \DIFaddbegin \DIFadd{$g1 = 75.1 \, Pa^{1/2}$,
$uWUE=2.82$ }\DIFaddend $\mu$-mol [C] Pa$^{0.5}$ J$^{-1}$ [ET]). These differences
...
\ref{idealized_sign}). \DIFdelbegin \DIFdel{CRO }\DIFdelend \DIFaddbegin \DIFadd{CROs }\DIFaddend evolved or were bred to prioritize GPP and
...
gradients in vapor pressure at the leaf scale will \DIFdelbegin \DIFdel{be so strong
that
they will begin to dominate leaf-scale plant strategies of stomatal
(partial) closure }\DIFdelend \DIFaddbegin \DIFadd{become very strong
as stomata reach their limits of closure in response to VPD
}\DIFaddend (parameterized with $g_1$). As a result, ET response will begin to
asymptote towards \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{a constant }\DIFaddend ecosystem-scale values \DIFdelbegin \DIFdel{irregardless of }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend leaf-scale
\DIFdelbegin \DIFdel{strategy and stomatal regulation
(parameterized with $g_1$)}\DIFdelend \DIFaddbegin \DIFadd{response to VPD asymptotes towards zero}\DIFaddend .  Therefore, plants with a low
...
apparent in the strong VPD dependence of \DIFdelbegin \DIFdel{ENF}\DIFdelend \DIFaddbegin \DIFadd{CSH}\DIFaddend , which has the lowest
slope parameter (\DIFdelbegin \DIFdel{$g_1=74.31$ }\DIFdelend \DIFaddbegin \DIFadd{$g_1=75.1$ }\DIFaddend Pa$^{1/2}$) (Figure \ref{idealized_sign}).
...
tendency to decrease ET with increasing VPD\DIFdelbegin \DIFdel{.  ENF ($g_1 = 74.31$
Pa$^{1/2}$) has by far the largest VPD response dependence, while CRO
($g_1 = 183.1$ Pa$^{1/2}$) has the smallest VPD dependence }\DIFdelend \DIFaddbegin \DIFadd{, as well as the strongest
VPD dependence of response}\DIFaddend . Figure \ref{idealized_sign} clearly shows,
...
engineered for photosynthesis rather than water saving. For \DIFdelbegin \DIFdel{forests and grass, }\DIFdelend \DIFaddbegin \DIFadd{DBF, EBF,
MF, GRA, and SAV }\DIFaddend more than half of the observed VPD are less than
VPD$_{crit}$, i.e. in conditions where plant response dominates. \DIFaddbegin \DIFadd{SAV
...
of the SAV ecoclimate. }\DIFaddend It is also important to note that \DIFaddbegin \DIFadd{for all PFTs,
}\DIFaddend even when atmospheric demand dominates, ET response to VPD is still
...
EBF, ENF\DIFaddbegin \DIFadd{, MF}\DIFaddend ) is generally about double the scaling terms for \DIFdelbegin \DIFdel{the }\DIFdelend other
PFTs which have lower surface roughness and generally smaller
aerodynamic conductance \DIFaddbegin \DIFadd{(GRA, CSH, CRO). The savannah (WSA, SAV) PFT's
scaling is somewhere between GRA, CSH, and CRO, and DBF, EBF, ENF, and
MF, due to higher variability and surface roughness}\DIFaddend .

\begin{figure}
  \DIFdelbeginFL %DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{./idealized_scale.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF >  \centering
  \DIFaddbeginFL \centerline{\includegraphics[width=1.4\textwidth]{./idealized_scale.pdf}}
...
\DIFdelFL{0.533 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{DBF }%DIFDELCMD < & %%%
\DIFdelFL{-0.10 }%DIFDELCMD < & %%%
\DIFdelFL{-0.013 }%DIFDELCMD < & %%%
\DIFdelFL{0.609 }%DIFDELCMD < \\ %%%
\DIFdelFL{EBF }%DIFDELCMD < & %%%
...
\DIFdelFL{0.740 }%DIFDELCMD < \\ %%%
\DIFdelFL{CSH }%DIFDELCMD < & %%%
\DIFdelFL{-0.29
}%DIFDELCMD < & %%%
\DIFdelFL{-0.224 }%DIFDELCMD < & %%%
\DIFdelFL{0.973 }%DIFDELCMD < \\ %%%
\DIFdelendFL \DIFaddbeginFL \input{stats.tex}
    \DIFaddendFL \hline
  \end{tabular}
  \label{stats}
\end{table}
...
observations. \DIFdelbegin \DIFdel{When listed by increasing }\DIFdelend \DIFaddbegin \DIFadd{The more water conservative PFTs generally exhibit
higher }\DIFaddend frequency of negative \DIFdelbegin \DIFdel{response, the PFTs are generally ordered as we would expect them to
be, from most water intensive to most water conservative: CRO (53\%),
DBF (61\%), EBF (67\%), GRA (69\%), ENF (74\%), and CSH (97\%). }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{ET response, especially when one factors
in the distribution of environmental VPD (e.g. SAV and WSA grow in
more arid climates, Figure \ref{idealized_sign}). }\DIFaddend In general the bulk
...
negative response\DIFaddbegin \DIFadd{, and shifts many of the other PFTs, which still
exhibit a high frequency of negative ET response, towards more frequent
occurrence of positive ET response }\DIFaddend than the theory and Figure
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{./test_sign.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=1.4\textwidth]{./test_sign.pdf}}
...
DBF \DIFaddbegin \DIFadd{and MF}\DIFaddend , CSH observations also match our theory to leading order\DIFaddbegin \DIFadd{,
albeit with a bit more variability as a function of VPD}\DIFaddend . Again, the
VPD dependence mostly determined from the slope parameter ($g_1$)
closely matches the medians in the observation bins\DIFdelbegin \DIFdel{and the
theoretical threshold is well reproduced (albeit off-scale)}\DIFdelend . The
VPD-independent, strongly negative response \DIFdelbegin \DIFdel{, which is determined by
uWUE, }\DIFdelend is also captured. For CSH,
there is \DIFdelbegin \DIFdel{extremely }\DIFdelend rare occurrence of observed positive ET response with VPD
(\DIFdelbegin \DIFdel{$\approx 3$}\DIFdelend \DIFaddbegin \DIFadd{$\approx 6$}\DIFaddend \%), even with uncertainty, so the sign of the
observations almost always matches the sign of the theory, which
states that ET response should always be negative.

\DIFdelbegin \DIFdel{As Table \ref{stats} suggested, the theory does not match as well for CRO as for DBF and CSH}\DIFdelend \DIFaddbegin \DIFadd{Biases between the theory and observations are similar for ENF, WSA,
CRO and SAV}\DIFaddend . At low VPD the \DIFdelbegin \DIFdel{VPD dependence (due to $g_1$)
is similar to the leading order behavior of the observations, but the theory
's curve is shifted towards more positive values relative to the
}\DIFdelend \DIFaddbegin \DIFadd{theory and the observations match
well. However, at high VPD (upper 10-20\% of }\DIFaddend observations\DIFdelbegin \DIFdel{, likely due to some bias in uWUE. Most strikingly, }\DIFdelend \DIFaddbegin \DIFadd{) the theory
is biased slightly towards positive response as compared to }\DIFaddend the
observations \DIFdelbegin \DIFdel{show a tendency to move back towards negative values at
high VPD, which given }\DIFdelend \DIFaddbegin \DIFadd{(Figure \ref{test_sign}). For ENF this slight bias could
...
towards more negative response, which cannot be captured by }\DIFaddend the
functional form of \DIFdelbegin \DIFdel{VPD dependence, is
behavior that is impossible for the theory to capture. This generalpositive bias of the theory explains the shift }\DIFdelend \DIFaddbegin \DIFadd{the sign term (Figure
...
similar limitations as for ENF, WSA, and SAV, except a larger
...
characterized by a consistent trend back }\DIFaddend towards negative ET response
\DIFdelbegin \DIFdel{observed in the bulk statistics in Table \ref{stats}. The
}\DIFdelend \DIFaddbegin \DIFadd{at higher VPD, which the functional form of our theory is incapable of
accounting for. As compared to CRO, WSA and SAV, the }\DIFaddend divergence
between the theory and the observations \DIFdelbegin \DIFdel{for CRO could have
been expected. For practical reasons, our theory does not include
important }\DIFdelend \DIFaddbegin \DIFadd{is far greater for GRA,
biasing 40-50\% of observations at higher VPD. In addition for the
potential for soil moisture to alter $uWUE$, there are other }\DIFaddend sources
of plant \DIFdelbegin \DIFdel{type heterogeneity within and across sites
for CRO. In agricultural landscape, the
footprint of an
eddy-covariance towers tend to not only observe a single species but
many different ones, with potentially varying water use strategies. We also }\DIFdelend \DIFaddbegin \DIFadd{heterogeneity specific to GRA (and some extent CRO) that may
alter $uWUE$ (or $g_1$) or invalidate other assumptions made in the
Methods section (Section \ref{methods}). We }\DIFaddend do not account for
\DIFdelbegin \DIFdel{seasonal changes in LAI, variability in crop
}\DIFdelend \DIFaddbegin \DIFadd{variability in plant }\DIFaddend height and surface roughness, \DIFaddbegin \DIFadd{or }\DIFaddend differences in
C3 vs C4 photosynthesis and water strategies, \DIFdelbegin \DIFdel{or differences in irrigation practices between
the sitesand years}\DIFdelend \DIFaddbegin \DIFadd{which we might expect to
vary substantially across sites, years, and season for GRA}\DIFaddend . These
deficiencies \DIFdelbegin \DIFdel{should }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend largely explain the inability of our theory to
...
C4 plants (at high environmental VPD) would explain \DIFaddbegin \DIFadd{the }\DIFaddend observed shift
back towards negative ET response at high VPD when all sites are
binned together, as in Figure \ref{test_sign}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The theory for GRA suffers from similar limitations as for CRO. GRA
...
strategy. However, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend hypothesize that the
theory would validate against observations much better \DIFdelbegin \DIFdel{when photosynthesis pathways are
accounted for.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The comparison between the theory and observations for ENF and EBF is
unique from the rest of the PFTs. The VPD dependence of the theory, determined by the slope parameter ($g_1$), is not as well captured by
...
\mbox{%DIFAUXCMD
\citep[from ][]{Lin_2015, Franks_2017}}%DIFAUXCMD
, which determines the VPD
dependence, and that deviations should be expected across sites
\mbox{%DIFAUXCMD
\citep{Lin_2015}}%DIFAUXCMD
. We could have fit our own constants so that the
theory would best match the observations, but our goal was to use a priori results whenever possible in order to avoid over-fitting and to
have a theoretical approach as independent to the observations as
possible for further confirmation of the validity of the approach}\DIFdelend \DIFaddbegin \DIFadd{if these
sources of variability were accounted for, at a cost of increased
complexity and analytical opacity}\DIFaddend . 


While the above discussion shows that our theory has some limitations
when applied to some PFTs, especially for \DIFdelbegin \DIFdel{crops and }\DIFdelend grasslands, it does well for
DBF\DIFaddbegin \DIFadd{, MF }\DIFaddend and CSH PFTs, and captures the response at non-extreme VPD
\DIFdelbegin \DIFdel{for ENF and EBF. It is useful to also compare our
...
drought indices. Again, as in our
discussion of the theoretical
results, the PET sensitivity to VPDwill be constant at about 3.5. In
...
and dryness responseat the land surface, as is done in
several
drought indices. Howeverour theory (Equation \ref{et}) could be used
with published uWUE and $g1$ data, as an alternative to PET to estimating land response and dryness, and would more accurately
account for the response to atmospheric drying}\DIFdelend \DIFaddbegin \DIFadd{(VPD < $\approx$ 70th-90th percentiles) for
...
can be explained and analyzed using Equation \ref{et}}\DIFaddend .

%DIF >  It is useful to also compare our theory, the observed uncertainty in
%DIF >  the ET response, and PET, as PET is often used as a proxy for ET in
%DIF >  hydrometeorological studies and drought indices. Again, as in our
%DIF >  discussion of the theoretical results, the PET sensitivity to VPD
%DIF >  will be constant at about 3.5. In Figure \ref{test_sign} the ET
%DIF >  response to shifts in dryness is always far smaller than 3.5 in the
%DIF >  observations, even with all uncertainty included. So the PET
%DIF >  response to atmospheric dryness is outside the range of all
%DIF >  observable responses. This casts doubt on PET's usefulness for
%DIF >  capturing the leading order behavior of water budget and dryness
%DIF >  response at the land surface, as is done in several drought
%DIF >  indices. However our theory (Equation \ref{et}) could be used with
%DIF >  published uWUE and $g1$ data, as an alternative to PET to estimating
%DIF >  land response and dryness, and would more accurately account for the
%DIF >  response to atmospheric drying.
\DIFaddbegin 

\DIFaddend \subsection{Observed ET response to VPD}
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=\textwidth]{./data_scatter.png}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=1.4\textwidth]{./data_scatter.png}}
  \DIFaddendFL \caption{Scatter plots of observed
...
EBF, ENF, \DIFdelbegin \DIFdel{and GRA }\DIFdelend \DIFaddbegin \DIFadd{MF, SAV and WSA }\DIFaddend are also water conservative, but show some
occurrence of positive ET response to VPD, particularly at higher
observed VPD, as the theory predicts. \DIFaddbegin \DIFadd{GRA, while generally water
...
scaled by $g_a$, and in general the \DIFdelbegin \DIFdel{sign term shifts towards positive values }\DIFdelend \DIFaddbegin \DIFadd{occurrence of positive ET response
increases }\DIFaddend as VPD increases. The willingness with which a given plant type
evolved to use water dictates the occurrence of positive versus
negative ET response. Water conservative \DIFdelbegin \DIFdel{plants }\DIFdelend \DIFaddbegin \DIFadd{ecosystems }\DIFaddend are highly effective
...
function. For CRO\DIFaddbegin \DIFadd{, MF }\DIFaddend and DBF, soil water content only matters for
...
stressed\DIFaddbegin \DIFadd{, suggesting that observed departures from the theory for CRO
are due to other factors (i.e. photosynthesis pathway) than soil
moisture}\DIFaddend . For ENF\DIFdelbegin \DIFdel{and EBF, }\DIFdelend \DIFaddbegin \DIFadd{, EBF, SAV, and WSA, }\DIFaddend systematic SWC-induced biases in
$\sigma \cdot uWUE$ emerge in about the lowest \DIFdelbegin \DIFdel{30}\DIFdelend \DIFaddbegin \DIFadd{20-30}\DIFaddend \% of SWC
conditions\DIFdelbegin \DIFdel{. In
crontast }\DIFdelend \DIFaddbegin \DIFadd{, although large variability in the SWC-$\sigma \cdot uWUE$
...
\DIFadd{In contrast }\DIFaddend to ENF, CRO, and DBF, for GRA the relationship between
...
and our theory is limited \DIFaddbegin \DIFadd{for the majority of environmental
conditions}\DIFaddend . It is therefore not \DIFdelbegin \DIFdel{suprising }\DIFdelend \DIFaddbegin \DIFadd{surprising }\DIFaddend that our theory tested
poorly \DIFdelbegin \DIFdel{againt }\DIFdelend \DIFaddbegin \DIFadd{against }\DIFaddend the data for GRA, relative to \DIFdelbegin \DIFdel{ENF, EBF,
DBF, and CSH }\DIFdelend \DIFaddbegin \DIFadd{to the other PFTs
}\DIFaddend (Section \ref{testing}). For all other PFTs \DIFdelbegin \DIFdel{occurence of soil
mosture }\DIFdelend \DIFaddbegin \DIFadd{occurrence of soil
moisture }\DIFaddend impacts was rare enough to not manifest itself in bulk
statistics and \DIFdelbegin \DIFdel{plots. A possible exception is CSH, for which there are
only two sites, which precludes comment on the exact PFT-wide
relationship}\DIFdelend \DIFaddbegin \DIFadd{figures}\DIFaddend .

\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{./swc_boxplot.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=1.4\textwidth]{./swc_boxplot.pdf}}
  \DIFaddendFL \caption{$\sigma \cdot uWUE$ as a function of PFT and SWC. Each box
    plot represents 5\% of all observations. \DIFaddbeginFL \DIFaddFL{Note that the highest
...
assumption) and observations. \DIFaddbegin \DIFadd{It is also perhaps not coincidental that
the portion of $\sigma \cdot uWUE$ affected by low soil moisture
observations is similar to the portion of observations that to not
...
our theory in Section \ref{testing}, particularly for GRA. \DIFaddbegin \DIFadd{By
...
\DIFadd{The theory described in Section \ref{sign_term} indicates that for a
...
controlling for environmental conditions. In particular, from
...
\DIFadd{There is a theoretical basis for the square root VPD dependence in
...
loss, which observations also generally support \mbox{%DIFAUXCMD
\citep{Lloyd_1991,
  MEDLYN_2011, Lin_2015, Zhou_2014, Zhou_2015, Medlyn_2017}}%DIFAUXCMD
. However,
some purely empirical results which fit the exponent of the VPD
...
on stomata theory, may not always function optimally \mbox{%DIFAUXCMD
\citep{Zhou_2015,
  Lin_2018}}%DIFAUXCMD
. Specifically with regards to $uWUE$, one would not expect
...
would also expect lower amounts of transpiration. \mbox{%DIFAUXCMD
\citet{Zhou_2015}}%DIFAUXCMD
's
...
sites. Interpreting \mbox{%DIFAUXCMD
\citet{Lin_2018}}%DIFAUXCMD
's results, which also show
variance in the empirical exponent of the VPD dependence of the
stomatal conductance model, is more difficult as \mbox{%DIFAUXCMD
\citet{Lin_2018} }%DIFAUXCMD
do
not handle GPP/A dependence of stomatal conductance in a directly
analogous manner to the optimal theory in \mbox{%DIFAUXCMD
\citet{MEDLYN_2011} }%DIFAUXCMD
and
\mbox{%DIFAUXCMD
\citet{Medlyn_2017}}%DIFAUXCMD
. Regardless, given that these recent results on
the relationship between VPD, GPP, and ET \mbox{%DIFAUXCMD
\citep{MEDLYN_2011,
  Zhou_2014, Zhou_2015, Medlyn_2017} }%DIFAUXCMD
...
ET-VPD dependence. By introducing $n$ and $m$ we can free our stomatal
conductance model from assumptions about VPD dependence:
}\begin{linenomath*}
  \begin{equation}
    \DIFadd{g_s = \frac{R \, T}{P} 1.6 \left(1 + \frac{g_*}{VPD^m}\right) \frac{*WUE \; ET}{c_a \; VPD^n},
    \label{m_n}
  }\end{equation}
\end{linenomath*}
\DIFadd{where:
}\[\DIFadd{*WUE = \frac{GPP}{ET}VPD^n,}\] \DIFadd{and $g_*$ is a generic slope parameter
...
($g_s$, with respect to VPD: }\input{d2_solutions.tex} \DIFadd{With this
result we've defined the family of curves separating concave up from
...
\begin{figure}
  ...
  \caption{ \DIFaddFL{Solutions corresponding to inflection points between
...
\end{figure}
 ...
 \begin{enumerate} 
  \item \DIFadd{For optimal behavior (n, m = 1/2) the ET-VPD curve will be
    ...
    to plant type.
  }\item \DIFadd{For all physically possible exponents of VPD dependence ($n,
    m$), whether the solution is concave up or concave down does not
    depend on $uWUE$.
  }\item \DIFadd{In general, increasing the exponent of VPD dependence
...
\DIFadd{While it is possible that in the future some other form of
VPD dependence is derived, at present \mbox{%DIFAUXCMD
\cite{MEDLYN_2011} }%DIFAUXCMD
and
\mbox{%DIFAUXCMD
\cite{Zhou_2014} }%DIFAUXCMD
firmly established n=m=1/2 as the most likely
...
slightly open \mbox{%DIFAUXCMD
\citep{De_2013, Nikinmaa_2013, Ryan_2014}}%DIFAUXCMD
. Furthermore,
even if complete stomatal closure were possible, cuticular water loss
and }[\DIFadd{at the ecosystem-scale}] \DIFadd{direct soil evaporation are still sources
...
\DIFadd{This analysis allows us to understand the theoretical shape of the ET
...
non-stationarity of }[\DIFadd{solar forced}] \DIFadd{environmental conditions over the
...
water conservative, while DBF, EBF, \DIFdelbegin \DIFdel{ENF, GRA}\DIFdelend \DIFaddbegin \DIFadd{SAV, GRA, MF, WSA, ENF }\DIFaddend and CSH are
progressively more water conservative (more likely to reduce ET in
response to increasing VPD). \DIFaddbegin \DIFadd{SAV and WSA exhibit positive ET response
...
atmospheric demand (VPD) relative to other PFTs. }\DIFaddend Observations of ET
...
respond to \DIFdelbegin \DIFdel{global change-induced shifts in VPD , which should increase
in the future}\DIFdelend \DIFaddbegin \DIFadd{VPD perturbations. We show that given optimal stomatal
...
VPD dependence, which we currently believe is 1/2 \mbox{%DIFAUXCMD
\citep{MEDLYN_2011,
  Zhou_2014}}%DIFAUXCMD
. Our results are applicable to understanding the impact
of  expected increases in VPD
induced by global change}\DIFaddend .  Plant physiological responses to direct
...
drying)\DIFdelbegin \DIFdel{. Additionally,
we
derive an explicit conceptual framework for ET response as a function
of two plant type parameters, $g_1$ and uWUE (e.g. }\DIFdelend \DIFaddbegin \DIFadd{, which is consistent with recent observational analysis
\mbox{%DIFAUXCMD
\citep[e.g.,][]{Rigden_2017}}%DIFAUXCMD
. Feedbacks between the land and the
...
in the atmospheric boundary layer (}\DIFaddend Equation \ref{et}). \DIFdelbegin \DIFdel{This framework can be used to understand ET response to a
range of environmental change, and forms a foundation for resolving
varying }\DIFdelend \DIFaddbegin \DIFadd{Our approach
can be extended to examine varying plant }\DIFaddend response to more nuanced
consideration of plant type and climate. Any plant physiological
heterogeneity \DIFaddbegin \DIFadd{or feedback }\DIFaddend that can be conceptualized with shifts in $g_1$
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.][]{Lin_2015} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.][]{Lin_2015, Medlyn_2017} }%DIFAUXCMD
}\DIFaddend and/or uWUE
\citep[e.g.][]{Zhou_2014} \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend representable within our framework, which
