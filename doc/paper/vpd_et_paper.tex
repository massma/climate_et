...
Note that $uWUE$ is fit on the ecosystem scale in \citet{Zhou_2015} so GPP in \ref{uwue} is really $A\cdot \text{LAI}$. This leads to the cancelation of LAI in addition to uWUE in Equation \ref{medlyn}. We also recognize that our use of $uWUE$ introudces considerable uncertainty, so we added an uncertainty parameter $\sigma = f(t, \text{site})$ modifying $uWUE$. The meaning of this parameter will be elaborated on after the derivation of $\frac{\partial \; ET}{\partial \; D}$.
