...
%DIF DEL agu-james-original-submission.tex   Wed Sep  4 09:38:13 2019
%DIF ADD agu-james-submission.tex            Thu Sep  5 13:40:16 2019
...
plants; and 3) \DIFdelbegin \DIFdel{due to }\DIFdelend plant type, with crops more likely to exhibit a
...
cases \DIFdelbegin \DIFdel{(e.g. plant to plant interaction and highly specialized
photosynthesis strategies like Crassulacean acid metabolism
photosynthesis)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite<e.g. plant to plant interaction, thermal regulation with
...
the \DIFdelbegin \DIFdel{the }\DIFdelend VPD dependence of ET while keeping other variables fixed. This
...
% \begin{linenomath*}
  ...
  \begin{equation}
    g_{1}  \propto \sqrt{\frac{3 \, \Gamma^* \,
        \lambda}{1.6}},
    \DIFdelbegin \footnote{\DIFdel{Note this expression has units of of
      (mmol mol$^{-1}$)$^{1/2}$, but this can be converted to
      Pa$^{1/2}$ using the ideal gas law.}}
    %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \label{slope}
  \end{equation}
% \end{linenomath*}


  where \DIFaddbegin \DIFadd{g$_1$ is presented in units of $\sqrt{\text{mol fraction}}$ for
  convenience (which can be converted to $\sqrt{Pa}$ using the ideal
  gas law and ambient pressure), }\DIFaddend $\Gamma^*$ is the CO$_2$ compensation
...
% \end{linenomath*}
  ...
  independent, which is supported by data (\DIFdelbegin \DIFdel{not shown}\DIFdelend \DIFaddbegin \DIFadd{see supplemental material}\DIFaddend ).
  ...
  additional assumption in the formulation of uWUE \cite{Zhou_2014,
    Zhou_2015} and \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Medlyn_2017}}%DIFAUXCMD
's }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend stomatal conductance model \DIFaddbegin \DIFadd{of
  \mbox{%DIFAUXCMD
\citeA{Medlyn_2017} }%DIFAUXCMD
}\DIFaddend is that direct soil evaporation contributions to
  ...
  high soil moisture\DIFdelbegin \DIFdel{and ponding, }\DIFdelend \DIFaddbegin \DIFadd{, ponding, and interception, }\DIFaddend but VPD is generally
  ...
  Because Eq. (\ref{d_et}) is a partial derivative, \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend utility is as
...
\begin{table}
  \caption{Variable quantities in the ET response to VPD. Each value
    ...
    provided \DIFdelbeginFL \DIFdelFL{in cases where the quantities }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for values of g$_1$ and uWUE, which }\DIFaddendFL are directly derived
    ...
    correspond to approximately the 85th percentile. \DIFaddbeginFL \DIFaddFL{Values for $T$
    and g$_a$ are calculated from FLUXNET-2015 data (see supplemental
    material for description), rounding the 15th, 50th, and 85th
    percentile to the nearest 5$^o$C and 0.005 m/s, respectively.}\DIFaddendFL }
  \label{param_varying}
  \centering
  \footnotesize
...
VPD. However, crops, \DIFaddbegin \DIFadd{shrubs, and grass are exceptions; they exhibit an
uWUE different than the median value in Table
\ref{param_varying}. Crops, }\DIFaddend which we expect to prioritize carbon
...
\citeA{Medlyn_2017}}%DIFAUXCMD
's }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend ecosystem-scale results \DIFaddbegin \DIFadd{of \mbox{%DIFAUXCMD
\citeA{Medlyn_2017} }%DIFAUXCMD
}\DIFaddend did not display the
...
is a robust relationship between g$_1$ and plant type in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Lin_2015}}%DIFAUXCMD
's }\DIFdelend \DIFaddbegin \DIFadd{the
}\DIFaddend leaf-scale results \DIFdelbegin \DIFdel{, and that \mbox{%DIFAUXCMD
\citeA{Zhou_2015}}%DIFAUXCMD
's }\DIFdelend \DIFaddbegin \DIFadd{of \mbox{%DIFAUXCMD
\citeA{Lin_2015}}%DIFAUXCMD
, and the }\DIFaddend ecosystem-scale
results \DIFaddbegin \DIFadd{of \mbox{%DIFAUXCMD
\citeA{Zhou_2015} }%DIFAUXCMD
}\DIFaddend rely on the same underlying theory as \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Medlyn_2017}}%DIFAUXCMD
's results}\DIFdelend \DIFaddbegin \DIFadd{in
\mbox{%DIFAUXCMD
\citeA{Medlyn_2017}}%DIFAUXCMD
}\DIFaddend , the ambiguous results in \citeA{Medlyn_2017}
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Lin_2015}}%DIFAUXCMD
's results }\DIFdelend \DIFaddbegin \DIFadd{the results of \mbox{%DIFAUXCMD
\citeA{Lin_2015} }%DIFAUXCMD
}\DIFaddend (e.g. Fig. 2) to the ecosystem scale
...
dependence. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Zhou_2015}}%DIFAUXCMD
's results }\DIFdelend \DIFaddbegin \DIFadd{The results from \mbox{%DIFAUXCMD
\citeA{Zhou_2015} }%DIFAUXCMD
}\DIFaddend corroborate this: they
...
AmeriFlux sites. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeA{Lin_2018}}%DIFAUXCMD
's results }\DIFdelend \DIFaddbegin \DIFadd{Results in \mbox{%DIFAUXCMD
\citeA{Lin_2018} }%DIFAUXCMD
}\DIFaddend also show variance in the
...
\begin{figure}
  ...
  \caption{ Solutions corresponding to inflection points between
    ...
    up and concave down results are labeled\DIFaddbeginFL \DIFaddFL{: for each curve, the
    parameter space ``below'' the curve corresponds to a concave up
    relationship, while the parameter space ``above'' the curve
    corresponds to a concave down relationship}\DIFaddendFL .}
  \label{concave}
\end{figure}
...
complete stomatal closure were possible, cuticular water loss and \DIFdelbegin %DIFDELCMD < [%%%
\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend at
the ecosystem-scale\DIFdelbegin %DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend direct soil evaporation are still sources of ET
...
time scale of plant response and the non-stationarity of \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{solar
forced}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{(solar
forced) }\DIFaddend environmental conditions over the relatively coarse (half
...
\DIFaddend We derived a new form of Penman Monteith using the concept of
...
Graduate Research Fellowship under Grant No. DGE 16-44869\DIFaddbegin \DIFadd{. We would
like to thank Emma Robinson and one anonymous reviewer for thoughtful
comments that greatly improved the strength of the manuscript}\DIFaddend .
