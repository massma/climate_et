...
\begin{frame}{Consequences of theory - ``sign'' term}
  ...
  \item $R_{air}$, $c_s$, and $\gamma$ are approximately constant (relative to $\sqrt{VPD}$)
...
\footnotesize
\[ \frac{\partial \, ET}{\partial \, VPD} = \text{scaling} \cdot  \left(\frac{ c_p}{R_{air}} - \frac{\gamma c_s }{1.6 \; R*\; \text{ uWUE }} \left( \frac{1}{g_1} - \frac{3\sqrt{VPD}}{2 g_1^2} +  \mathcal{O}\left(\frac{\sqrt{VPD}}{g_1}\right)^{2} \right) \right)\]
\large
and:
...
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}[plain]{Test theory with FLUXNET data - CSH}
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[width=\paperwidth]{csh_eee.png}
            };
        \end{tikzpicture}
     \end{frame}
}
...
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}[plain]{Test theory with FLUXNET data - ENF}
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[width=\paperwidth]{enf_eee.png}
            };
        \end{tikzpicture}
     \end{frame}
}
...
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}[plain]{Test theory with FLUXNET data - DBF}
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[width=\paperwidth]{dbf_eee.png}
            };
        \end{tikzpicture}
     \end{frame}
}
...
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}[plain]{Test theory with FLUXNET data - CRO}
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[width=\paperwidth]{cro_eee.png}
            };
        \end{tikzpicture}
     \end{frame}
}
...
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}[plain]{Test theory with FLUXNET data - GRA}
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[width=\paperwidth]{gra_eee.png}
            };
        \end{tikzpicture}
     \end{frame}
}
...
\begin{frame}{Summary}
  ...
  \item For plants evolved to prioritize production over water conservation (high uWUE and $g_1$), atmospheric demand is more likely to dominate.
  \item But, for all plant types except for crops decreases in ET (plant response dominates) with increases in VPD are common (plant response dominates). Therefor, the hydrometeorlogists perspective is often wrong, and drought indices built upon assumptions about atmospepheric demand are unrealistic for vegetated surfaces.
  \item Our form of Penman-Monteith is capable of estimating ET more realistically and could be used as a replacement for PET in drought indices.
  \end{itemize}
\end{frame}
