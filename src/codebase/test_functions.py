...
def plot_et_curve_onlyPM(mean_row, min_row, max_row, index, _df):
  """plots the et curve given a row of df"""
  vpd = np.linspace(min_row.vpd, max_row.vpd)
  et_orig = pm_et_orig(mean_row, vpd=vpd)
  plt.figure()
...
def plot_et_curve_onlyLeaf(mean_row, min_row, max_row, index, _df):
  """plots the et curve given a row of df"""
  vpd = np.linspace(min_row.vpd, max_row.vpd)
  leaf = et_leaf(mean_row, vpd=vpd)
  plt.figure()
...
def run_all_tests(dfs):
  ...
  [plot_et_curve_onlyuWUE(dfs['mean'].loc[index],\
                          ...
                          dfs['95'].loc[index], index, _df)\
   for index in dfs['mean'].index]
  [plot_et_curve_onlyLeaf(dfs['mean'].loc[index],\
                          dfs['5'].loc[index],\
                          dfs['95'].loc[index], index, _df)\
   for index in dfs['mean'].index]
  [plot_et_curve_onlyPM(dfs['mean'].loc[index],\
                          dfs['5'].loc[index],\
                          dfs['95'].loc[index], index, _df)\
   for index in dfs['mean'].index]
